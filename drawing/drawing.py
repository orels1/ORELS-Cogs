    ...
    def __init__(self, bot):
        ...
        self.patchnote = """
            **Settings are here!**
            ...
            With `[p]drawing setsign` you can change default sign on images generated by `[p]text` command. Also, if you set it to 'username', it will use user's name as a sign.
            Example:
            ```[p]drawing setsign Sinon```

            With `[p]drawing setbg` you can change default background on images generated by `[p]text` and `[p]userbar` commands. You can always rever to default by adding 'default' to the end of the command.
            Example:
            ```[p]drawing setbg default```

            **Both commands are available to bot owner exclusively**

            Also, added a default avatar for plebs without any :3
