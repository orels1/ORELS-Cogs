...
A new build type was added - `yt` it will build a youtube thumbnail. It has two options 'small' and 'border' which are responsible for layout type and frame color respectively
Example:
```[p]build yt small #ff00ff```
If you want to build with standard layout, but still have the custom frame color, just pass "" as the first option
Example:
```[p]build yt "" #ff00fff```
All this is described in respective `help` command: `[p]help build yt`

You can also permanently set the layout type and frame color by the new `[p]build set youtube` command. There you can just specify wich option to change and pass the value.
Example:
```[p]build set youtube small True```
That will force compact layout for all the generated images

For the list of options and values, you can always check the `[p]help build set` and `[p]help build set youtube` commands

**All settings can be changed ONLY by the bot owner**
...
"""
    ...
    async def setsign(self, *, sign):
        ...
        if len(sign.strip()) <= 40:
            self.drawing_settings["bot_sign"] = sign.strip()
    ...
    async def setbg(self, ctx, default = None):
        ...
        if default is not None:
            self.drawing_settings["background"] = "data/drawing/bg.png"
            fileIO("data/drawing/settings.json", "save", self.drawing_settings)
            ...
            if success:
                ...
                if success:
                    ...
                    if bg_image.size == (400,100):
                        self.drawing_settings["background"] = "data/drawing/custom_bg"
                        fileIO("data/drawing/settings.json", "save", self.drawing_settings)
    ...
    async def text(self, ctx, *, text):
        ...
        if len(text) > 24:
            ...
            result = Image.open(self.drawing_settings["background"]).convert('RGBA')
            ...
            # get sign
            sign = self.drawing_settings["bot_sign"]
            if sign == "username":
                ...
                if bg_image.size[0] >= 1280 and bg_image.size[1] >= 720:
                    ...
                    if len(title) < 35:
                        ...
                        if len(small) == 0:
                            ...
                            if "youtube" in self.drawing_settings["build"].keys():
                                if "small" in self.drawing_settings["build"]["youtube"].keys() and self.drawing_settings["build"]["youtube"]["small"] is True:
                                    # get fonts
                        ...
                        if "youtube" in self.drawing_settings["build"].keys():
                            if "frame" in self.drawing_settings["build"]["youtube"].keys():
                                border = self.drawing_settings["build"]["youtube"]["frame"]
    ...
    @_set.command()
    async def youtube(self, option, *, value):
        ...
        if option in options.keys():
            if ("values" in options[option].keys() and value in options[option]["values"]) or options[option]["type"] == "Any":
                ...
                if "youtube" not in self.drawing_settings["build"].keys():
                    self.drawing_settings["build"]["youtube"] = {}

                self.drawing_settings["build"]["youtube"][option] = value
                fileIO("data/drawing/settings.json", "save", self.drawing_settings)
    ...
    async def userbar(self, ctx, background = None):
        ...
        if success:
            ...
            if background is None:
                ...
                if self.drawing_settings["background"] == "data/drawing/bg.png":
                    server_image = server_image.resize(size=(100,100))               
                    result.paste(server_image, (0,0))
                else:
                    bg_image = Image.open(self.drawing_settings["background"]).convert('RGBA')
